...
#define EJERCICIO_1 (17)
#define EJERCICIO_2 (18)
#define EJERCICIO_3 (19)

#define TEST (EJERCICIO_3)
...
/* Take Semaphore, UART (or output) & LED toggle thread */
static void vHandlerTask(void *pvParameters)// esta tiene que ser la tarea que se habilita con el semaforo
{
	portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;
	/* As per most tasks, this task is implemented within an infinite loop.
	 *
	...
	while (1) {
        ...
        /* To get here the event must have occurred.  Process the event (in this
         * case we just print out a message). */
        DEBUGOUT("me habilito el semaforo.\r\n");
...
static void vtask_1(void *pvParameters) // la tarea periodica que me piden
{
    /* As per most tasks, this task is implemented within an infinite loop. */
	while (1) {
		/* This task is just used to 'simulate' an interrupt.  This is done by
        ...
        /* Generate the interrupt, printing a message both before hand and
         * afterwards so the sequence of execution is evident from the output. */
        DEBUGOUT("tarea periodica que hace la interrupcion.\r\n");
        mainTRIGGER_INTERRUPT();

    }
}

static void vtask_3(void *pvParameters) // la tarea que se habilita con la cola
{
	char *pcMessageToPrint;
	static char cBuffer[mainMAX_MSG_LEN];
...
void vSoftwareInterruptHandler(void)	// supongo que eta es el semaforo que me piden
{
	portBASE_TYPE xHigherPriorityTaskWoken = pdFALSE;

    /* 'Give' the semaphore to unblock the task. */
	DEBUGOUT("entro por la interrupcion.\n\n");
	DEBUGOUT("apunto de sincronizar el semaforo.\n\n");
    xSemaphoreGiveFromISR(xBinarySemaphore, &xHigherPriorityTaskWoken);
...
int main(void)
{
    ...
    /* Before a semaphore is used it must be explicitly created.  In this example
     * a binary semaphore is created. */
    vSemaphoreCreateBinary(xBinarySemaphore); // cree el semaforo

    /* Before a queue is used it must be explicitly created.  The queue is created
      * to hold a maximum of 5 character pointers. */
     xPrintQueue = xQueueCreate( 5, sizeof( char * ) ); // cree la cola
    ...
    if (xBinarySemaphore != (xSemaphoreHandle) NULL) {
         ...
         * This is created with a priority below the handler task to ensure it will
         * get preempted each time the handler task exits the Blocked state. */
        xTaskCreate(vtask_1, (char *) "Periodic", configMINIMAL_STACK_SIZE, NULL,
        			(tskIDLE_PRIORITY + 1UL), (xTaskHandle *) NULL);

        xTaskCreate(vtask_3, (char *) "cola", configMINIMAL_STACK_SIZE, NULL,
        			(tskIDLE_PRIORITY + 0UL), (xTaskHandle *) NULL);
...
void vSoftwareInterruptHandler(void)	// supongo que eta es el semaforo que me piden
{
    ...
    /* 'Give' the semaphore to unblock the task. */
	DEBUGOUT("entro por la interrupcion.\n\n");
	DEBUGOUT("apunto mandar mensaje a la cola.\n\n");
...
static void vTask1(void *pvParameters)
{
	...
	while (1) {
		...
		vTaskDelayUntil(&xLastExecutionTime, 13000 / portTICK_RATE_MS);
